...
#include "zrshcoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zRSH ought to be enough for anybody." - Bill Gates, 2017
    ui->zRSHpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzRSHSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzRSH->setEnabled(false);
        ui->pushButtonMintzRSH->setToolTip(tr("zRSH is currently disabled due to maintenance."));

        ui->pushButtonSpendzRSH->setEnabled(false);
        ui->pushButtonSpendzRSH->setToolTip(tr("zRSH is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zRSHpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzRSH_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zRSH is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zRSH...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zRSH in ") +
...
void PrivacyDialog::on_pushButtonSpendzRSH_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zRSH is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zRSH
        sendzRSH();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zRSH
    sendzRSH();
}

void PrivacyDialog::on_pushButtonZRSHCoinControl_clicked()
{
    ZRSHCoinControlDialog* zRSHControl = new ZRSHCoinControlDialog(this);
    zRSHControl->setModel(walletModel);
    zRSHControl->exec();
}

void PrivacyDialog::setZRSHCoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzRSHSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzRSH()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid RSHCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zRSHpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zRSH is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " RSHCOIN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zRSH</b>";
    ...
    // use mints from zRSH selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZRSHCoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZRSHCoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zrshcoin selector in case it was used
    ZRSHCoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zRSH Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " RSHCoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zRSH Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zRSH </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zRSH "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zRSH "));
    ui->labelzRSHAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzRSHSyncStatus->setVisible(fShow);
}
