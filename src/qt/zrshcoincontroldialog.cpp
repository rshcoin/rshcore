...
#include "ui_zrshcoincontroldialog.h"
...
std::list<std::string> ZRSHCoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZRSHCoinControlDialog::listMints;

ZRSHCoinControlDialog::ZRSHCoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZRSHCoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZRSHCoinControlDialog::~ZRSHCoinControlDialog()
{
    delete ui;
}

void ZRSHCoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZRSHCoinControlDialog::updateList()
{
...
void ZRSHCoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZRSHCoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZRSHCoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZRSHCoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZRSHCoinControlDialog::GetSelectedMints()
{
...
void ZRSHCoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
